...
# Alloc memory for an "array" of 10 integers
#
# Print each one, move using pointer arithmetic.
...
def main():
    ...
    allocation = malloc(sizeof(int32_t) * 10)
    ...
    #  `int.deref` is equivalent to `int[0]` and to `(int + 0).deref`
    int_array = (int32_ptr_t)(allocation)
    ...
    printf("Writing 0, 10, 20, 30, ..., 90 to each position of the array\n")
    # Let's write some values
    for i in range(10):
        # Note that we don't need `.deref` when indexing. This is also known as implicit deref.
        int_array[i] = i * 10

    printf("First element using int_array.deref: %d\n", int_array.deref)
    ...
    #   - ...and so on
    printf(
        "Third element using pointer arithmetic (int_array + 2).deref: %d\n",
        (int_array + 2).deref,
    )
    ...
    #
    # For this simulation, we'll just crash.
    #
    ...
    # Trying to dereference the element (either for reading or writing) will crash the program.
    # Uncomment any of the following lines to crash this example.
    # int_array[10] = 123
